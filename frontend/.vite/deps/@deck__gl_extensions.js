import { E as COORDINATE_SYSTEM, N as fp64, S as project_default, T as memoize, X as defaultLogger, a as WebMercatorViewport, n as deepEqual, q as equals, r as Model, s as fp64LowPart, t as mergeShaders, z as dist } from "./shader-BokkZAiK.js";
...
//#endregion
//#region node_modules/@deck.gl/extensions/dist/brushing/shader-module.js
var uniformBlock$4 = `\
...
var shader_module_default$2 = {
  ...
  `
	...
	getUniforms: (opts) => {
		if (!opts || !("viewport" in opts)) return {};
...
//#endregion
//#region node_modules/@deck.gl/extensions/dist/brushing/brushing-extension.js
var defaultProps$7 = {
	getBrushingTarget: {
		type: "accessor",
		value: [0, 0]
...
var BrushingExtension = class extends LayerExtension {
	getShaders() {
		return { modules: [shader_module_default$2] };
	}
	initializeState(context, extension) {
		const attributeManager = this.getAttributeManager();
		if (attributeManager) attributeManager.add({ brushingTargets: {
			size: 2,
			stepMode: "dynamic",
		...
		if (context.deck) context.deck.eventManager.on({
...
BrushingExtension.defaultProps = defaultProps$7;
BrushingExtension.extensionName = "BrushingExtension";
//#endregion
//#region node_modules/@deck.gl/extensions/dist/data-filter/shader-module.js
var uniformBlock$3 = `\
...
function getUniforms$1(opts) {
	if (!opts || !("extensions" in opts)) return {};
...
function getUniforms64(opts) {
	if (!opts || !("extensions" in opts)) return {};
	const uniforms = getUniforms$1(opts);
...
//#endregion
//#region node_modules/@deck.gl/extensions/dist/data-filter/aggregator.js
var AGGREGATE_VS = `\
#version 300 es
...
function getFramebuffer(device, useFloatTarget) {
	if (useFloatTarget) return device.createFramebuffer({
		width: 1,
		height: 1,
...
function getModel(device, bufferLayout, shaderOptions, useFloatTarget) {
	...
	if (useFloatTarget) shaderOptions.defines.FLOAT_TARGET = 1;
...
//#endregion
//#region node_modules/@deck.gl/extensions/dist/data-filter/data-filter-extension.js
var defaultProps$6 = {
	getFilterValue: {
		type: "accessor",
		value: 0
...
var DataFilterExtension = class extends LayerExtension {
	...
	initializeState(context, extension) {
		...
		const { categorySize, filterSize, fp64 } = extension.opts;
		if (attributeManager) {
			if (filterSize) attributeManager.add({ filterValues: {
				size: filterSize,
				type: fp64 ? "float64" : "float32",
				stepMode: "dynamic",
				accessor: "getFilterValue"
			} });
			if (categorySize) attributeManager.add({ filterCategoryValues: {
				size: categorySize,
				stepMode: "dynamic",
		...
		if (attributeManager && extension.opts.countItems) {
			...
			attributeManager.add({ filterVertexIndices: {
	...
	updateState({ props, oldProps, changeFlags }, extension) {
		...
		if (this.state.filterModel) {
			...
			if (filterNeedsUpdate) this.setState({ filterNeedsUpdate });
		}
		if (attributeManager?.attributes.filterCategoryValues) {
			if (attributeManager.attributes.filterCategoryValues.needsUpdate() || !deepEqual(props.filterCategories, oldProps.filterCategories, 2)) this.setState({ categoryBitMask: null });
			if (changeFlags.dataChanged) {
				this.setState({ categoryMap: Array(categorySize).fill(0).map(() => ({})) });
	...
	draw(params, extension) {
		...
		if (!this.state.categoryBitMask) extension._updateCategoryBitMask.call(this, params, extension);
		...
		if (this.state.categoryBitMask) dataFilterProps.categoryBitMask = this.state.categoryBitMask;
		this.setShaderModuleProps({ dataFilter: dataFilterProps });
		if (filterNeedsUpdate && onFilteredItemsChange && filterModel) {
			const { attributes: { filterValues, filterCategoryValues, filterVertexIndices } } = this.getAttributeManager();
			...
			let count = 0;
			for (let i = 0; i < color.length; i++) count += color[i];
			onFilteredItemsChange({
				id: this.id,
				count
			});
	...
	*/
	_updateCategoryBitMask(params, extension) {
		...
		const categoryBitMask = new Uint32Array([
			0,
			0,
			0,
			0
		...
		for (let c = 0; c < categoryFilters.length; c++) {
			...
			for (const category of categoryFilter) {
				...
				} else defaultLogger.warn(`Exceeded maximum number of categories (${maxCategories})`)();
			}
		}
	...
	*/
	_getCategoryKey(category, channel) {
		...
		if (!(category in categoryMap)) categoryMap[category] = Object.keys(categoryMap).length;
		return categoryMap[category];
	}
};
DataFilterExtension.defaultProps = defaultProps$6;
DataFilterExtension.extensionName = "DataFilterExtension";
//#endregion
//#region node_modules/@deck.gl/extensions/dist/fp64/project64.glsl.js
...
`;
//#endregion
//#region node_modules/@deck.gl/extensions/dist/fp64/project64.js
...
//#endregion
//#region node_modules/@deck.gl/extensions/dist/fp64/fp64-extension.js
/** @deprecated Adds the legacy 64-bit precision to geospatial layers. */
var Fp64Extension = class extends LayerExtension {
	getShaders() {
		const { coordinateSystem } = this.props;
		if (coordinateSystem !== "lnglat" && coordinateSystem !== "default") throw new Error("fp64: coordinateSystem must be LNGLAT");
		return { modules: [project64_default] };
	}
	draw(params, extension) {
...
//#endregion
//#region node_modules/@deck.gl/extensions/dist/path-style/shaders.glsl.js
var dashShaders = { inject: {
	"vs:#decl": `
...
var scatterplotDashShaders = { inject: {
	"vs:#decl": `
in vec2 instanceDashArrays;
out vec2 vDashArray;
`,
...
var textBackgroundDashShaders = { inject: {
	"vs:#decl": `
in vec2 instanceDashArrays;
out vec2 vDashArray;
`,
...
//#endregion
//#region node_modules/@deck.gl/extensions/dist/path-style/path-style-extension.js
var defaultProps$5 = {
	getDashArray: {
		type: "accessor",
		value: [0, 0]
...
var PathStyleExtension = class extends LayerExtension {
	...
	getLayerType(layer) {
		if ("pathTesselator" in layer.state) return "path";
		const layerName = layer.constructor.layerName;
		if (layerName === "ScatterplotLayer") return "scatterplot";
		if (layerName === "TextBackgroundLayer") return "textBackground";
		return null;
	}
	isEnabled(layer) {
	...
	getShaders(extension) {
		...
		if (layerType === "scatterplot" || layerType === "textBackground") {
			if (!extension.opts.dash) return null;
			return { modules: [{
				name: "pathStyle",
				inject: layerType === "scatterplot" ? scatterplotDashShaders.inject : textBackgroundDashShaders.inject,
		...
		if (extension.opts.dash) {
			...
			if (extension.opts.highPrecisionDash) defines.HIGH_PRECISION_DASH = true;
		}
		if (extension.opts.offset) result = mergeShaders(result, offsetShaders);
		const { inject } = result;
		return {
			modules: [{
				name: "pathStyle",
	...
	initializeState(context, extension) {
		const attributeManager = this.getAttributeManager();
		const layerType = extension.getLayerType(this);
		if (!attributeManager || !layerType) return;
		if (extension.opts.dash) attributeManager.addInstanced({
			instanceDashArrays: {
				size: 2,
				accessor: "getDashArray"
			},
			...layerType === "path" && extension.opts.highPrecisionDash ? { instanceDashOffsets: {
				size: 1,
				accessor: "getPath",
				transform: extension.getDashOffsets.bind(this)
			} } : {}
		});
		if (layerType === "path" && extension.opts.offset) attributeManager.addInstanced({ instanceOffsets: {
			size: 1,
			accessor: "getOffset"
		} });
	}
	updateState(params, extension) {
		...
		if (extension.opts.dash) {
			const layerType = extension.getLayerType(this);
			if (layerType === "scatterplot" || layerType === "textBackground") {
				const pathStyleProps = { dashGapPickable: Boolean(this.props.dashGapPickable) };
	...
	getDashOffsets(path) {
		...
		for (let i = 0; i < geometrySize - 1; i++) {
			...
			if (i > 0) result[i] = result[i - 1] + dist(prevP, p);
			prevP = p;
		}
...
PathStyleExtension.defaultProps = defaultProps$5;
PathStyleExtension.extensionName = "PathStyleExtension";
//#endregion
//#region node_modules/@deck.gl/extensions/dist/fill-style/shader-module.js
var uniformBlock$2 = `\
...
function getPatternUniforms(opts) {
	...
	if ("project" in opts) {
		...
		const { commonOrigin: coordinateOriginCommon } = project_default.getUniforms(opts.project);
...
//#endregion
//#region node_modules/@deck.gl/extensions/dist/fill-style/fill-style-extension.js
var defaultProps$4 = {
...
var FillStyleExtension = class extends LayerExtension {
	...
	getShaders(extension) {
		...
		return { modules: [extension.opts.pattern && patternShaders].filter(Boolean) };
	}
	initializeState(context, extension) {
		if (!extension.isEnabled(this)) return;
		const attributeManager = this.getAttributeManager();
		if (extension.opts.pattern) attributeManager.add({
			fillPatternFrames: {
				size: 4,
				stepMode: "dynamic",
		...
		this.setState({ emptyTexture: this.context.device.createTexture({
	...
	updateState({ props, oldProps }, extension) {
		if (!extension.isEnabled(this)) return;
		if (props.fillPatternMapping && props.fillPatternMapping !== oldProps.fillPatternMapping) this.getAttributeManager().invalidate("getFillPattern");
	}
	draw(params, extension) {
		if (!extension.isEnabled(this)) return;
...
FillStyleExtension.defaultProps = defaultProps$4;
FillStyleExtension.extensionName = "FillStyleExtension";
//#endregion
//#region node_modules/@deck.gl/extensions/dist/clip/clip-extension.js
var defaultProps$3 = {
	clipBounds: [
		0,
		0,
...
var ClipExtension = class extends LayerExtension {
	getShaders() {
		...
		if (this.props.clipByInstance !== void 0) clipByInstance = Boolean(this.props.clipByInstance);
	...
	draw() {
		...
		const clipProps = {};
		if (this.state.clipByInstance) clipProps.bounds = clipBounds;
		else {
...
ClipExtension.defaultProps = defaultProps$3;
ClipExtension.extensionName = "ClipExtension";
//#endregion
//#region node_modules/@deck.gl/extensions/dist/collision-filter/shader-module.js
var vs$1 = `
...
var getCollisionUniforms = (opts) => {
	if (!opts || !("dummyCollisionMap" in opts)) return {};
...
//#endregion
//#region node_modules/@deck.gl/extensions/dist/collision-filter/collision-filter-pass.js
var CollisionFilterPass = class extends LayersPass {
	...
	getShaderModuleProps() {
		return {
			collision: { drawToCollisionMap: true },
			picking: {
				isActive: 1,
				isAttribute: false
			},
...
//#endregion
//#region node_modules/@deck.gl/extensions/dist/collision-filter/collision-filter-effect.js
var DOWNSCALE = 2;
var CollisionFilterEffect = class {
	...
	preRender({ effects: allEffects, layers, layerFilter, viewports, onViewportActive, views, isPicking, preRenderStats = {} }) {
		const { device } = this.context;
		if (isPicking) return;
		const collisionLayers = layers.filter(({ props: { visible, collisionEnabled } }) => visible && collisionEnabled);
		...
		if (needsRender) {
			...
			this.collisionFilterPass.renderCollisionMap(collisionFBO, {
				...
				shaderModuleProps: {
					...
					project: { devicePixelRatio: collisionFBO.device.canvasContext.getDevicePixelRatio() / DOWNSCALE }
				}
			});
		}
	}
	...
	*/
	_groupByCollisionGroup(device, collisionLayers) {
		...
		for (const layer of collisionLayers) {
			...
			if (!layer.isLoaded) channelInfo.allLayersLoaded = false;
		}
		for (const collisionGroup of Object.keys(channelMap)) {
			if (!this.collisionFBOs[collisionGroup]) this.createFBO(device, collisionGroup);
			if (!this.channels[collisionGroup]) this.channels[collisionGroup] = channelMap[collisionGroup];
		}
		for (const collisionGroup of Object.keys(this.collisionFBOs)) if (!channelMap[collisionGroup]) this.destroyFBO(collisionGroup);
		return channelMap;
	}
	getShaderModuleProps(layer) {
		...
		return { collision: {
			enabled: collisionEnabled && Boolean(collisionFBO),
			collisionFBO,
			dummyCollisionMap
		} };
	}
	cleanup() {
		...
		this.channels = {};
		for (const collisionGroup of Object.keys(this.collisionFBOs)) this.destroyFBO(collisionGroup);
		this.collisionFBOs = {};
...
//#endregion
//#region node_modules/@deck.gl/extensions/dist/collision-filter/collision-filter-extension.js
var defaultProps$2 = {
...
var CollisionFilterExtension = class extends LayerExtension {
	getShaders() {
		return { modules: [shader_module_default$1] };
	}
	draw({ shaderModuleProps }) {
		if (shaderModuleProps.collision?.drawToCollisionMap) this.props = this.clone(this.props.collisionTestProps).props;
	}
	initializeState(context, extension) {
		if (this.getAttributeManager() === null) return;
		this.context.deck?._addDefaultEffect(new CollisionFilterEffect());
		this.getAttributeManager().add({ collisionPriorities: {
			size: 1,
			stepMode: "dynamic",
...
CollisionFilterExtension.defaultProps = defaultProps$2;
CollisionFilterExtension.extensionName = "CollisionFilterExtension";
//#endregion
//#region node_modules/@deck.gl/extensions/dist/mask/shader-module.js
var uniformBlock$1 = `\
...
var getMaskUniforms = (opts) => {
	if (opts && "maskMap" in opts) return { mask_texture: opts.maskMap };
	return opts || {};
};
...
//#endregion
//#region node_modules/@deck.gl/extensions/dist/mask/mask-pass.js
var MASK_BLENDING = {
...
//#endregion
//#region node_modules/@deck.gl/extensions/dist/utils/projection-utils.js
function joinLayerBounds(layers, viewport) {
	...
	if (Number.isFinite(bounds[0])) return bounds;
	return null;
}
...
function getViewportBounds(viewport, zRange) {
	...
	} else viewportBoundsWorld = viewport.getBounds();
...
function getRenderBounds(layerBounds, viewport, zRange) {
	...
	const paddedBounds = doubleBounds(getViewportBounds(viewport, zRange));
	if (layerBounds[2] - layerBounds[0] <= paddedBounds[2] - paddedBounds[0] && layerBounds[3] - layerBounds[1] <= paddedBounds[3] - paddedBounds[1]) return layerBounds;
	return [
...
//#endregion
//#region node_modules/@deck.gl/extensions/dist/mask/mask-effect.js
var MaskEffect = class {
	...
	preRender({ layers, layerFilter, viewports, onViewportActive, views, isPicking }) {
		...
		if (viewport.resolution !== void 0) {
			defaultLogger.warn("MaskExtension is not supported in GlobeView")();
			return { didRender };
		}
	...
	_renderChannel(channelInfo, { layerFilter, onViewportActive, views, viewport, viewportChanged }) {
		...
		if (!oldChannelInfo) return didRender;
	...
	*/
	_sortMaskChannels(maskLayers) {
		...
		for (const layer of maskLayers) {
			...
			if (!channelInfo) {
				if (++channelCount > 4) {
					defaultLogger.warn("Too many mask layers. The max supported is 4")();
					continue;
				}
				channelInfo = {
					id,
...
//#endregion
//#region node_modules/@deck.gl/extensions/dist/mask/mask-extension.js
var defaultProps$1 = {
	maskId: "",
...
var MaskExtension = class extends LayerExtension {
	...
	getShaders() {
		...
		if (this.props.maskByInstance !== void 0) maskByInstance = Boolean(this.props.maskByInstance);
		this.state.maskByInstance = maskByInstance;
		return { modules: [shader_module_default] };
	}
	draw({ context, shaderModuleProps }) {
		...
		if (maskChannels && maskChannels[maskId]) {
			...
			if (fromCoordinateSystem === COORDINATE_SYSTEM.DEFAULT) fromCoordinateSystem = viewport.isGeospatial ? COORDINATE_SYSTEM.LNGLAT : COORDINATE_SYSTEM.CARTESIAN;
			const opts = {
				modelMatrix: null,
				fromCoordinateOrigin,
				fromCoordinateSystem
			};
			const bl = this.projectPosition([
				bounds[0],
				bounds[1],
				0
			], opts);
			const tr = this.projectPosition([
				bounds[2],
				bounds[3],
				0
			], opts);
			...
			if (maskId) defaultLogger.warn(`Could not find a mask layer with id: ${maskId}`)();
			maskProps.enabled = false;
		}
		this.setShaderModuleProps({ mask: maskProps });
	}
};
MaskExtension.defaultProps = defaultProps$1;
MaskExtension.extensionName = "MaskExtension";
//#endregion
//#region node_modules/@deck.gl/extensions/dist/terrain/shader-module.js
...
var uniformBlock = Object.keys(TERRAIN_MODE).map((key) => `const float TERRAIN_MODE_${key} = ${TERRAIN_MODE[key]}.0;`).join("\n") + `
...
var terrainModule = {
	name: "terrain",
	dependencies: [project_default],
	vs: uniformBlock + "out vec3 commonPos;",
  ...
  return;
	...
	getUniforms: (opts = {}) => {
		if ("dummyHeightMap" in opts) {
			...
			const { commonOrigin } = project_default.getUniforms(opts.project);
			...
			if (drawToTerrainHeightMap) {
				...
				sampler = (opts.isPicking ? terrainCover.getPickingFramebuffer() : terrainCover.getRenderFramebuffer())?.colorAttachments[0].texture;
				if (opts.isPicking) mode = TERRAIN_MODE.SKIP;
				if (sampler) {
					mode = mode === TERRAIN_MODE.SKIP ? TERRAIN_MODE.USE_COVER_ONLY : TERRAIN_MODE.USE_COVER;
					bounds = terrainCover.bounds;
				} else {
					sampler = dummyHeightMap;
					if (opts.isPicking && !terrainSkipRender) mode = TERRAIN_MODE.NONE;
				}
			}
			return {
				mode,
...
//#endregion
//#region node_modules/@deck.gl/extensions/dist/terrain/utils.js
...
//#endregion
//#region node_modules/@deck.gl/extensions/dist/terrain/terrain-cover.js
/**
...
var TerrainCover = class {
	...
	shouldUpdate({ targetLayer, viewport, layers, layerNeedsRedraw }) {
		if (targetLayer) this.targetLayer = targetLayer;
		const sizeChanged = viewport ? this._updateViewport(viewport) : false;
		let layersChanged = layers ? this._updateLayers(layers) : false;
		if (layerNeedsRedraw) {
			for (const id of this.layers) if (layerNeedsRedraw[id]) {
				layersChanged = true;
				break;
			}
		}
	...
	_updateLayers(layers) {
		...
		if (layers.length !== this.layers.length) needsRedraw = true;
		else for (let i = 0; i < layers.length; i++) if (layers[i].id !== this.layers[i]) {
			needsRedraw = true;
			break;
		}
		if (needsRedraw) this.layers = layers.map((layer) => layer.id);
		return needsRedraw;
	}
	...
	_updateViewport(viewport) {
		...
		if (!this.targetBoundsCommon) return false;
		const newZoom = Math.ceil(viewport.zoom + .5);
		...
		if (shouldRedraw) this.renderViewport = makeViewport({
			bounds: this.bounds,
			zoom: newZoom,
			viewport
		});
	...
	getRenderFramebuffer() {
		if (!this.renderViewport || this.layers.length === 0) return null;
		if (!this.fbo) this.fbo = createRenderTarget(this.targetLayer.context.device, { id: this.id });
		return this.fbo;
	}
	getPickingFramebuffer() {
		if (!this.renderViewport || this.layers.length === 0 && !this.targetLayer.props.pickable) return null;
		if (!this.pickingFbo) this.pickingFbo = createRenderTarget(this.targetLayer.context.device, {
			id: `${this.id}-picking`,
			interpolate: false
		});
...
function getIntersectingLayers(sourceTile, layers) {
	return layers.filter((layer) => {
		...
		if (tile) return intersect(sourceTile.boundingBox, tile.boundingBox);
		return true;
	});
}
...
function intersect(b1, b2) {
	if (b1 && b2) return b1[0][0] < b2[1][0] && b2[0][0] < b1[1][0] && b1[0][1] < b2[1][1] && b2[0][1] < b1[1][1];
	return false;
}
//#endregion
//#region node_modules/@deck.gl/extensions/dist/terrain/terrain-pass.js
...
var TerrainPass = class extends LayersPass {
	getRenderableLayers(viewport, opts) {
		...
		for (let i = 0; i < layers.length; i++) {
			...
			if (!layer.isComposite && drawParamsByIndex[i].shouldDrawLayer) result.push(layer);
		}
		return result;
	}
	...
	getShaderModuleProps(layer, effects, otherShaderModuleProps) {
		return { terrain: { project: otherShaderModuleProps.project } };
	}
};
//#endregion
//#region node_modules/@deck.gl/extensions/dist/terrain/terrain-picking-pass.js
...
var TerrainPickingPass = class extends PickLayersPass {
	...
	renderTerrainCover(terrainCover, opts) {
		...
		if (terrainLayer.props.pickable) layers.unshift(terrainLayer);
		target.resize(viewport);
		const terrainAlpha = this.drawParameters[terrainLayer.id]?.blendColor?.[3] ?? 0;
		this.render({
			...opts,
			pickingFBO: target,
	...
	getLayerParameters(layer, layerIndex, viewport) {
		let parameters;
		if (this.drawParameters[layer.id]) parameters = this.drawParameters[layer.id];
		else {
	...
	getShaderModuleProps(layer, effects, otherShaderModuleProps) {
		return {
			...super.getShaderModuleProps(layer, effects, otherShaderModuleProps),
			terrain: { project: otherShaderModuleProps.project }
		};
	}
};
//#endregion
//#region node_modules/@deck.gl/extensions/dist/terrain/height-map-builder.js
...
var HeightMapBuilder = class {
	...
	*/
	getRenderFramebuffer() {
		if (!this.renderViewport) return null;
		if (!this.fbo) this.fbo = createRenderTarget(this.device, {
			id: "height-map",
			float: true
		});
		return this.fbo;
	...
	shouldUpdate({ layers, viewport }) {
		...
		if (!this.layersBoundsCommon) this.renderViewport = null;
...
//#endregion
//#region node_modules/@deck.gl/extensions/dist/terrain/terrain-effect.js
...
var TerrainEffect = class {
	...
	setup({ device, deck }) {
		...
		if (HeightMapBuilder.isSupported(device)) this.heightMap = new HeightMapBuilder(device);
		else defaultLogger.warn("Terrain offset mode is not supported by this browser")();
	...
	preRender(opts) {
		...
		if (!isPicking) {
			if (layers.filter((l) => l.state.terrainDrawMode === "offset").length > 0) this._updateHeightMap(terrainLayers, viewport, opts);
		}
	...
	getShaderModuleProps(layer, otherShaderModuleProps) {
		...
		if (this.isPicking && layer.props.operation.includes("terrain")) layer.state._hasPickingCover = Boolean(terrainCover?.getPickingFramebuffer());
		return { terrain: {
	...
	cleanup({ deck }) {
		...
		for (const terrainCover of this.terrainCovers.values()) terrainCover.delete();
		this.terrainCovers.clear();
		deck._removeDefaultShaderModule(terrainModule);
	}
	_updateHeightMap(terrainLayers, viewport, opts) {
		if (!this.heightMap) return;
		if (!this.heightMap.shouldUpdate({
	...
	_updateTerrainCovers(terrainLayers, drapeLayers, viewport, opts) {
		...
		for (const layer of drapeLayers) if (layer.state.terrainCoverNeedsRedraw) {
			layerNeedsRedraw[layer.id] = true;
			layer.state.terrainCoverNeedsRedraw = false;
		}
		for (const terrainCover of this.terrainCovers.values()) terrainCover.isDirty = terrainCover.isDirty || terrainCover.shouldUpdate({ layerNeedsRedraw });
		for (const layer of terrainLayers) this._updateTerrainCover(layer, drapeLayers, viewport, opts);
		if (!this.isPicking) this._pruneTerrainCovers();
	}
	_updateTerrainCover(terrainLayer, drapeLayers, viewport, opts) {
		...
		try {
			...
			if (this.isPicking || terrainCover.isDirty || isDirty) {
				...
				if (!this.isPicking) terrainCover.isDirty = false;
			}
		} catch (err) {
	...
	_pruneTerrainCovers() {
		...
		const idsToRemove = [];
		for (const [id, terrainCover] of this.terrainCovers) if (!terrainCover.isActive) idsToRemove.push(id);
		for (const id of idsToRemove) this.terrainCovers.delete(id);
	}
};
//#endregion
//#region node_modules/@deck.gl/extensions/dist/terrain/terrain-extension.js
var defaultProps = { terrainDrawMode: void 0 };
/** Allows layers to show/hide objects by a geofence. */
var TerrainExtension = class extends LayerExtension {
	getShaders() {
		return { modules: [terrainModule] };
	}
	initializeState() {
	...
	updateState(params) {
		...
		if (this.state.terrainDrawMode && props.terrainDrawMode === oldProps.terrainDrawMode && props.extruded === oldProps.extruded) return;
	...
	onNeedsRedraw() {
		...
		if (state.terrainDrawMode === "drape") state.terrainCoverNeedsRedraw = true;
	}
};
TerrainExtension.defaultProps = defaultProps;
TerrainExtension.extensionName = "TerrainExtension";
//#endregion
export { BrushingExtension, ClipExtension, CollisionFilterExtension, DataFilterExtension, FillStyleExtension, Fp64Extension, MaskExtension, PathStyleExtension, TerrainExtension as _TerrainExtension, project64_default as project64 };
